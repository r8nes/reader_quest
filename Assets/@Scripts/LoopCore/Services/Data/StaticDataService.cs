...
namespace ChooseReader.Data.Static
{
    public class StaticDataService : IStaticDataService
    {
        ...
        private Dictionary<string, LevelStaticData> _data;
        private Dictionary<WindowId, WindowConfig> _windowConfigs;

        public void Load()
        {
            _data = Resources.LoadAll<LevelStaticData>(LEVELS_PATH).ToDictionary(x => x.BookKey, x => x);
        ...
        public LevelStaticData LoadReaderData(string sceneKey) =>
            _data.TryGetValue(sceneKey, out LevelStaticData data)
            ? data
            : null;
